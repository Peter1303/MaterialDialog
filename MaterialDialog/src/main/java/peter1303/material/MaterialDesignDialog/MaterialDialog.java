...
import peter1303.material.MaterialDesignDialog.R;
...
public class MaterialDialog
{
    ...
    private int pId = -1, nId = -1, ntId= -1;
    private String pText, nText,ntText;
    View.OnClickListener pListener, nListener,ntListener;
	...
	public MaterialDialog setItems(List<String>list,final ListView.OnItemClickListener listener){
		this.ItemsList=list;
		this.listListener=listener;
		return this;
	}
	
	public MaterialDialog setItemsLongClick(final ListView.OnItemLongClickListener listener){
		this.listLongListener=listener;
		return this;
	}
    ...
    private class Builder {
        ...
        private Builder() {
            ...
            View contentView = LayoutInflater.from(mContext).inflate(R.layout.layout_materialdialog,null);
            ...
            mTitleView = (TextView) mAlertDialogWindow.findViewById(R.id.material_title);
            mMessageView = (TextView) mAlertDialogWindow.findViewById(R.id.material_message);
            mButtonLayout = (LinearLayout) mAlertDialogWindow.findViewById(R.id.buttonLayout);
            mPositiveButton = (Button) mButtonLayout.findViewById(R.id.material_Positive);
            mNegativeButton = (Button) mButtonLayout.findViewById(R.id.material_Negative);
            mNeutralButton = (Button) mButtonLayout.findViewById(R.id.material_Neutral);
			...
			if(ItemsList!=null){
				...
				setItems(ItemsList,listListener);
			}
			if(listListener!=null&&listView!=null){
            ...
            if (ntId != -1){
                ...
                mNeutralButton.setText(ntId);
                mNeutralButton.setOnClickListener(ntListener);
                if (isLollipop()){
                    mNegativeButton.setBackgroundResource(
                        android.R.color.transparent);
                }
            ...
            if (!isNullOrEmpty(ntText)){
                mNeutralButton.setVisibility(View.VISIBLE);
                mNeutralButton.setText(ntText);
                mNeutralButton.setOnClickListener(ntListener);
                if (isLollipop()) {
                    mNeutralButton.setBackgroundResource(
                        android.R.color.transparent);
                }
            }
            ...
            if (isNullOrEmpty(ntText) && ntId == -1) {
		...
		public void setItems(List<String>list,final ListView.OnItemClickListener listener){
			final ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(mContext,R.layout.layout_list,R.id.material_list_text);
		...
		public void setItemsOnLongClick(ListView listView,final ListView.OnItemLongClickListener listener){
			listView.setOnItemLongClickListener(listLongListener);
		}
